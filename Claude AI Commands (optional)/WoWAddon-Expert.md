...
- `securecallfunction(fn, ...)` - Call function in its own security context (use for calling Blizzard APIs from addon code)
...
- **Map canvas taint (12.0.0+):** Addon frames on `WorldMapFrame:GetCanvas()` cause taint. Use `nil` frame names, `EnableMouse(false)` + `EnableMouseMotion(false)`, hide when not needed. For map pins, use `CreateUnsecuredRegionPoolInstance` (not `CreateFramePool`) and pre-register in `WorldMapFrame.pinPools[templateName]`. `AddDataProvider` inserts addon objects as tainted keys; prefer manual pin lifecycle. Never `hooksecurefunc` on WorldMapFrame methods (use `EventRegistry` callbacks: `WorldMapOnShow`, `WorldMapOnHide`, `WorldMapMinimized`, `WorldMapMaximized`, `MapCanvas.MapSet`). Never `hooksecurefunc` on FlightMapFrame methods (taints pin pipeline; defer with `C_Timer.After(0, ...)`). Pin `OnReleased` fires before `OnLoad` on first acquire — must be nil-safe. No `MapCanvasPinTemplate` exists; use plain frames with mixin attribute.
